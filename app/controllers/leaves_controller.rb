class LeavesController < ApplicationController
  ...
  def admin_index
    ...
    @pending_leaves.each {|leave| @response << {title: leave.user.name, start: leave.leave_date_from.to_time.iso8601, end: ((leave.leave_date_to.to_date)+ 1).to_time.iso8601 , url: "/leaves/#{leave.id}/leave_decision", color: '#e6ac00' } }
    @accepted_leaves.each {|leave| @response << {title: leave.user.name, start: leave.leave_date_from.to_time.iso8601, end: ((leave.leave_date_to.to_date)+ 1).to_time.iso8601 , url: "/leaves/#{leave.id}/leave_decision", color: 'green' } }
    @rejected_leaves.each {|leave| @response << {title: leave.user.name, start: leave.leave_date_from.to_time.iso8601, end: ((leave.leave_date_to.to_date)+ 1).to_time.iso8601 , url: "/leaves/#{leave.id}/leave_decision", color: 'red' } }
